...
class Deliverydate extends Template {
    ...
    Context $context, Data $helper, DateTime $date, Timezone $timeZone, PricingHelper $priceHelper, OrderCollectionFactory $order, Session $customerSession, Cart $cart,Resolver $localeResolver,QuoteRepository $quoteRepository, ResourceConnection $resourceConnection, array $data = []
    ) {
        $this->_order = $order;
    ...
    public function dayOff()
    {
        ...
        if(!empty($commonDays) && count($commonDays) > 0 )
        {
             $missingNumbers = array_diff($completeSet, $commonDays);
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/getAllItemCart.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        $logger->info('text message');
        $cartId = $this->_cart->getQuote()->getId();
        $logger->info(print_r($cartId,true));

        $connection = $this->resourceConnection->getConnection();
        
        // Define the quote table name
        $quoteTable = $this->resourceConnection->getTableName('mg_quote');
        
        // Prepare and execute the query
        $select = $connection->select()
            ->from($quoteTable, ['shipping_arrival_date'])
        ...
        // Fetch the result
        $shippingArrivalDate = $connection->fetchOne($select, $bind);

        $logger->info(print_r($shippingArrivalDate,true));
        return $shippingArrivalDate ? $shippingArrivalDate : null;
