...
class Data extends AbstractHelper {
	...
	public function __construct(
		Context $context, EncryptorInterface $encryptor, ModuleListInterface $moduleList, StoreManagerInterface $storeManager, Config $resourceConfig, ObjectManagerInterface $objectmanager, ReinitableConfigInterface $coreConfig, Website $web, CollectionFactory $collectionFactory, TimezoneInterface $timezone, PricingHelper $priceHelper, TransportBuilder $transportBuilder, DateTime $date, Repository $assetRepo, SerializerInterface $serializer, StateInterface $inlineTranslation, OrderRepositoryInterface $orderRepository, \Magento\Directory\Model\Currency $currency, ProductMetadataInterface $metaData, Manager $moduleManager
	) {
...
//                        $order = $this->orderRepository->get($order_data['entity_id']);
	...
	public function getDeliverydateConfig($deliveryDate) {
		$datesArray = [];
		if(!empty($deliveryDate))
		{
			...
			while ($currentDate <= $deliveryDate) {
				// Add the current date to the array in d-m-Y format
