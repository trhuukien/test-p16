...
class Data extends AbstractHelper
{
 ...
 /**
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/ProductView.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        $logger->info('Product View');
       
        if (isset($productDetails)) {
            $deliveryDays = $productDetails->getData('delivery_days');
            $orderProcessingTime = $productDetails->getAttributeText('order_processing_time');
            if(!empty($deliveryDays) && !empty($orderProcessingTime))
            {
                ...
                {
                    $logger->info('Current date > Formatted cut of time');
                ...
                $logger->info('Off days');
                $logger->info(print_r($offdays,true));

                $dayOffs = $this->deliverydate->getDayOffs();

                $logger->info(print_r($dayOffs,true));

                $daysOffArray = explode(',', $dayOffs);
                for ($i = 0; $i < $orderProcessingTime; ) {
                    // Check if the current date is a valid processing day
                    if (!in_array($currentDate->format('w'), $daysOffArray) && !in_array($currentDate->format('d-m-Y'), $offdays)) {
                        $logger->info('Processing Date: ' . $currentDate->format('d-m-Y')); // Log valid processing days
                        $i++; // Increment processing day count
                    }
                
                    // Move to the next day
                    $currentDate->modify('+1 day'); 
                
                    // Skip both weekends and specific off days
                    while (in_array($currentDate->format('d-m-Y'), $offdays)) {
                        $currentDate->modify('+1 day');
                    }
                
                    $logger->info('Next Available Date: ' . $currentDate->format('d-m-Y'));
                }
                ...
                $logger->info('Current Date final');
                $logger->info(print_r($currentDate,true));

                $logger->info('after day off');
                $logger->info(print_r($currentDate,true));
                $logger->info('Delivery Daye Number');
                $logger->info(print_r($deliveryDayNumbers,true));
                // Loop through each delivery day name
                foreach ($deliveryDayNumbers as $deliveryDayOfWeek) {
                ...
                $logger->info('Nearest Day');
                $logger->info(print_r($nearestDate->format('Y-m-d'),true));
    ...
    function getNextAvailableDay($currentDate, $offDates) {
        while (in_array($currentDate->format('Y-m-d'), $offDates)) {
