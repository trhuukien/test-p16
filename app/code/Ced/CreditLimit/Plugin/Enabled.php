<?php
/**
 * CedCommerce
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the End User License Agreement (EULA)
 * that is bundled with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://cedcommerce.com/license-agreement.txt
 *
 * @category    Ced
 * @package     Ced_CreditLimit
 * @author 		CedCommerce Core Team <connect@cedcommerce.com>
 * @copyright   Copyright CedCommerce (http://cedcommerce.com/)
 * @license     http://cedcommerce.com/license-agreement.txt
 */
namespace Ced\CreditLimit\Plugin;

class Enabled
{
  /**
   * 
   * @param \Magento\Checkout\Model\Cart $cart
   * @param \Magento\Customer\Model\Session $session
   * @param \Ced\CreditLimit\Helper\Data $credithelper
   */
	public function __construct(
			\Magento\Checkout\Model\Cart $cart,
			\Magento\Customer\Model\Session $session,
			\Ced\CreditLimit\Helper\Data $credithelper
				
	){
		$this->helper = $credithelper;
		$this->cart = $cart;
		$this->customerSession = $session;
	}
    /**
     * {@inheritdoc}
     */
    public function afterIsSatisfiedBy($subject, $result,$paymentMethod)
    { 
    	
        if($this->customerSession->isLoggedIn()){
        	
        	$creditdata = $this->helper->getCustomerCreditLimit($this->customerSession->getCustomerId());
        	$isOfflineHide = $this->helper->getConfigValue('b2bextension/credit_limit/hide_offline');
        	$flag = false;
        	$discountTotal = 0;
        	
        	foreach ($this->cart->getQuote()->getAllItems() as $item){
        		if($item->getSku()==\Ced\CreditLimit\Model\CreditLimit::CREDIT_LIMIT_SKU){
        			$flag = true;
        			break;
        		}
        		$discountTotal += $item->getDiscountAmount();
        	}
        	     	
        	$basetotal = $this->cart->getQuote()->getBaseGrandTotal();
        	
        	$paymentamount = $basetotal-$discountTotal;
        	if($creditdata->getRemainingAmount() < $paymentamount || $flag){
        	    if($paymentMethod=='paybycredit')
        		    return false;
        	}
        	return $result;
        }
            return $result;
    }
}