...
class ServiceOutputProcessor
{
        ...
        if ($serviceClassName == 'Magento\Sales\Api\OrderRepositoryInterface' && $serviceMethodName == 'getList') {
            if (!empty($result) && isset($result['items'])) {
                foreach ($result['items'] as $key => $order) {
                            ...
                            foreach($options as $_options) {
                                foreach($_options as $option) {
                                    if (isset($option['option_sku'])) {
                                        $sku_option0 = str_replace('-'.$option['option_sku'],'', $sku_option0);
                                    }
                    ...
                    $result['items'][$key]['customer_note'] = $this->getCustomerNote($order['entity_id']);
                }
            }
            ...
            $result['customer_note'] = $this->getCustomerNote($result['entity_id']);
            $result['estimated_delivery_date_datetime'] = $this->getDateTime($result['entity_id']);
        }
        return $result;
    }
    ...
    public function getOption($optionId, $optionValue, $store_id){
        ...
        if (isset($option['type']) && in_array($option['type'],['drop_down','checkbox','radio','multiple'])) {
            ...
            if (is_array($option)) {
                ...
                // Handle the case when $option is not an array (e.g., set a default value or log an error).
                // You can return an empty array or any other appropriate value based on your requirements.
                return [];
            }
        }
    }
     ...
     * @return string
     */
    public function getDateTime($orderId)
    {
        $connection = $this->resourceConnection->getConnection();
        $select = $connection->select()
            ->from(
                ['so' => $this->resourceConnection->getTableName('sales_order')],
                ['estimated_delivery_date_datetime']
            )->where(
                'so.entity_id = ?',
                $orderId
            );
        $customer_note = $connection->fetchOne($select);
     ...
     * @return array
     */
    public function getCustomerNote($orderId)
    {
        $connection = $this->resourceConnection->getConnection();
        $select = $connection->select()
            ->from(
                ['so' => $this->resourceConnection->getTableName('sales_order')],
                ['sparsh_order_comments']
            )->where(
                'so.entity_id = ?',
                $orderId
            );
