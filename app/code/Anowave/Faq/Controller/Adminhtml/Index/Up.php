...
class Up extends Position
{
	...
	public function execute()
	{
		 ...
		 * @var int $previous
		 */
		$prev = $this->getPreviousId($model);
		
		if ($prev)
		{
			$this->swap($model->getId(), $prev);
		}
		
		return $this->redirectBack();
	}
	
	private function getPreviousId(\Anowave\Faq\Model\Item $model)
	{
		if ($model->getId())
		{
			$sql = 'SELECT 
