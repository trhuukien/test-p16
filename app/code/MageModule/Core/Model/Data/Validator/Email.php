...
//TODO create php unit test
/**
 * Class Email
 *
 * @package MageModule\Core\Model\Data\Validator
 */
class Email implements \MageModule\Core\Model\Data\ValidatorInterface
{
    /**
    ...
    public function validate(array $data)
    {
        ...
        if (count($invalidValues) === 1) {
            $message = sprintf('The following value is not a valid e-mail address: %s', current($invalidValues));
        } elseif (count($invalidValues) > 1) {
            $message = sprintf('The following values are not valid e-mail addresses: %s', implode(', ', $invalidValues));
        }
